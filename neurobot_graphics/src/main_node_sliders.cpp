#include "../include/neurobot_graphics/main_node_sliders.hpp"

MainNode::MainNode()
    : Node("main_node"), frequency_(0.1), amplitude_(0.5), is_pulse_(false), time_(0.0), mode_(1)
{
...
void MainNode::timer_callback()
{
    ...
    if (mode_ == 1)
    {
        ...
        publisher_PositionReference->publish(message);
    }
    else if (mode_ == 2)
    {
        double freq = frequency_ * 2.0 * M_PI;
        message.data = amplitude_ * std::sin(freq * time_);
        publisher_EffortReference->publish(message);
    }
    time_ += 0.01;
}
